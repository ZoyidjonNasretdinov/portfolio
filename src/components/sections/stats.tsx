...
const Statistics = () => {
  return (
    <section className="bg-primary w-full rounded-[1vw] p-[1vw] text-textPrimary shadow-lg mobile:p-[4vw] relative overflow-hidden mobile:rounded-[3vw]">
      {/* Statistics list */}
      <ul className="grid grid-cols-3 gap-[1vw] justify-center mobile:grid-cols-2 mobile:grid-rows-2 mobile:gap-[3vw]">
        {statsData.map(({ count, label, Icon }, index) => (
          ...
          >
            ...
            <p className="bg-gradient-to-b from-white to-gray-400 text-transparent bg-clip-text 
              text-[3vw] font-extrabold mobile:text-[15vw]">
              {count}
              <span className="text-textSecondary">+</span>
            </p>

            {/* Label with icon */}
            <span className="flex items-center gap-[0.5vw] text-[0.6vw]  text-white bg-[#141414] px-[1vw] py-[0.5vh] rounded-[0.5vw] mobile:text-[3vw] mobile:rounded-[3vw] mobile:px-[3vw]">
              <Icon className="text-textSecondary text-[1.2vw] mobile:text-[5.5vw]" />
              {label}
            </span>
          </li>
        ))}
